...
x = x / max(abs(x));
% If the sample is coming from a *.wav file, activate next lines removing
% some of the previous ones
%[xorigin fs] = audioread('Clasica.wav');
%x=xorigin(10000: 13840);
%x = x / max(abs(x));
...
for OFFSET = 1:384:length(x) - 384;
    % If we need only a step in the loop use OFFSET=1
   S = [];
